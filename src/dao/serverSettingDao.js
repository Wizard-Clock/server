...
async function getAllServerSettings() {
    return await new Promise((resolve, reject) => {
        db.dbConnector.all('SELECT * FROM server_settings', (err, rows) => {
            if (err)
                reject(err);
            else
                resolve(rows);
...
async function updateServerSetting(serverSetting) {
    await db.dbConnector.run(`INSERT INTO server_settings (setting_name, value) VALUES (?, ?) ON CONFLICT(setting_name) DO UPDATE SET value=?`, [
        serverSetting.name,
        serverSetting.value,
        serverSetting.value
    ]);
